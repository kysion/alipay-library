...
```
/
├── alipay_consts              常量定义：组件专用常量或公共类型及变量定义。
├── alipay_controller              接口处理：接收/解析用户输入参数的入口/接口层。
│   ├── merchant          Alipay商户基本操作。
│   └── xxx.go            Alipay数据库基础表操作及网关通知消息等。
├── alipay_model          结构模型：数据结构管理模块，管理数据实体对象，以及输入与输出数据结构定义。
│   ├── alipay_dao           数据访问：数据访问对象，这是一层抽象对象，用于和底层数据库交互，仅包含最基础的 CURD 方法
│   ├── alipay_do            领域对象：用于dao数据操作中业务模型与实例模型转换，由工具维护，用户不能修改。
│   ├── alipay_entity        数据模型：数据模型是模型与数据集合的一对一关系，由工具维护，用户不能修改。
│   ├── alipay_enum          枚举定义：枚举类型及变量定义。
│   └── alipay_hook          Hook定义：钩子函数定义。
├── alipay_service        模块接口：用于组件解耦的接口定义层。具体的接口实现在logic中进行注入。
│   ├── gateawy.go            Alipay网关及数据库基础。
│   └── merchant.go           Alipay商户基本操作。
├── api                   对外接口：接口定义
│   └── alipay_v1             对外提供服务的输入/输出数据结构定义，包含多主体接口定义。
│       ├── alipay_consumer           Alipay消费者相关接口。
│       ├── alipay_merchant_v1        Alipay商户相关接口。
│       ├── alipay_third_v1           Alipay第三方应用相关接口。
...
├── internal          组件逻辑：组件逻辑存放目录。通过internal特性对外部隐藏可见性。
│   └── logic         模块封装：组件逻辑封装管理。
│       ├── gateawy            Alipay网关及数据库基础逻辑代码。
│       ├── internal           Alipay拓展逻辑代码。
│           └── aliyun              Aliyun拓展逻辑代码。
│       └── merchant           Alipay商户基本操作逻辑代码。
...
特性：
> 封装了Alipay网关，用于接收和发送消息。<br>
> 封装了Alipay小程序开发管理Api <br>
> 实现了Alipay 授权、登陆、支付、分账、 转账等... <br>
